...
/**
 * Add new application form page controller
 * @author JC Dela Cerna Jr. May 2019
 */
class AddNewApplicationFormController extends BaseController {
    ...
    public function index() {
        try {
            ...
            if ($this->input->get('yes') == '1') {
                $this->parse('application/add_new_application_with_existing_coa_form', 'Add New Application Form', $data);
            } else {
        ...
        try {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (isset($_FILES['dully_sined_appointment_or_designation']['name']) &&
                    ...
                    $isSave = $this->AddNewApplicationPageModel->insertApplicationData($appData);
                    if ($isSave) {
                        // Add a comment to notify the pco...
                        $id = $isSave; // Application id.
                        $comment = null;
                        ...
                        if ($isSubmitted) {
                            ...
                            $dispositionLog = [
                                ...
                                'document_no' => 'PCO-' . $documentNo . '-ID-' . $id, 
                                'document_date' => $date,
                                'date_time' => $date,
                                'remarks' => $comment,
                            ];
