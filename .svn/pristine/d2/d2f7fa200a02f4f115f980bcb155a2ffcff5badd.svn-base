...
class ApplicationRenewedPageController extends BaseController {
    ...
    public function index() {
        try {
            ...
            // tthis is for the existing old coa 
            // if the application was system genrated meaning the
            // application is not existing old coa
    ...
    public function getCOAFromOriginId($id) {
        ...
        if($application['old_coa_attachment']) {
            ...
            return $this->ApplicationRenewalPageModel->getCOANoByID($application['origin']);
        }
    }
    ...
    public function updateRenewedApplicationDetails() {
        try {
            ...
            if ($this->form_validation->run() == FALSE) {
               ...
               $is_updated = $this->AddNewApplicationPageModel->updateApplicationData($application_data, $post['application_id']);
               if($is_updated) {
                   ...
                   if($isRenew) {
                       $response = ['status' => 2, 'message' => 'Successfully updated!', 'redirectUrl' => base_url('application-page')];
                   }
               }
            }
        } catch (Exception $err) {
