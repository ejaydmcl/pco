...
class ApplicationDetailsPageModel extends CI_Model {
     ...
     * @return {boolean} True of false
     */
    public function createCOANoForRenewal($approvedByUserFk,$applicationOriginID, $applicationFk) {
        $dateApproved = date('Y-m-d H:i:s'); // The approved date of the application
        // date("Y-m-d H:i:s", strtotime("+1 years", strtotime('2014-05-22 10:35:10'))); //2015-05-22 10:35:10
        //  date("Y-m-d H:i:s", strtotime("+1 months", strtotime('2014-05-22 10:35:10')));//2014-06-22 10:35:10
        // date("Y-m-d H:i:s", strtotime("+1 days", strtotime('2014-05-22 10:35:10')));//2014-05-23 10:35:10
        ...
        $this->db->select('a.sys_gen');
        $this->db->from('application a');
        $this->db->where('a.id', $applicationFk);
        $query = $this->db->get();
        $result = $query->row_array();
        if($result['sys_gen'] == 0) {
            // Not a system generated
            $coa = $this->getCOANo();
        } else {
            // Format COA No. 2020-RXI-0000
            $coa = $this->getCOANoForRenewal($applicationOriginID);
        }
        $lastData = substr($coa, 12);
...
//        }
        ...
        // Get the user region.
        // This function if for regional director only.
        $region = isset($this->session->userdata['user']['region']) ? $this->session->userdata['user']['region'] : null;
        $_region = $this->utils->getRegionPrifix($region);
        // Sample: COA No. 2020-your region-0001
        if(isset($res['coa_no'])) { // TODO
            $id = substr($res['coa_no'], 13 + strlen($_region) + 1); // Omit the string.
            $generatedID = intval($id) + 1; // Increment the coa id number.
            return 'COA No. '. $year . '-'. $_region .'-'. str_pad($generatedID, 4, '0', STR_PAD_LEFT); // return the string.
        } else {
            return 'COA No. '. $year . '-'. $_region .'-'. str_pad(1, 4, '0', STR_PAD_LEFT); // return the string.
        }
    }
    ...
    public function getListOfUsersByDesignation($user) {
        ...
        switch ($designation['designation_id']) {
            case SECTION_CHIEF:
                    ...
                    $this->db->where('e.region', $user['region']);
                    $this->db->where('u.employee_fk IS NOT NULL');
                    $this->db->where('u.designation_fk IS NOT NULL');
                    $this->db->where('ur.role_fk',EMPLOYEE);
    ...
    public function getApplicationStatus($id) {
        ...
        //echo 'getApplicationStatus'. var_dump($designation);
...
//        $this->db->where('c.user_fk <>', $this->session->userdata['user']['id']);
    ...
    public function recordTheNotification($data) {
        ...
        if($data['flag'] == EVALUATOR) {
            ...
            else if($designationType['designation_id'] == EVALUATOR){
                $notification = ['user_fk'=>$data['user_fk'],'application_fk'=>$data['application_fk'],
                    'sender_id'=>$data['sender_id'],'receiver_id'=>$receiverId,'comments_fk' => $commentId, 
                    'flag_1' => 1,'flag_2' => 0,'flag_3' => 1,'flag_4' => 1,'flag_5' => 1,'date_created'=>$data['date_created']]; 
            } 
        ...
        if($data['flag'] == SECTION_CHIEF) { 
            ...
            if($designationType['designation_id'] == EVALUATOR){
                ...
                // Set the evaluator.
                $this->db->update('application', ['evaluator_fk' => $receiverId], ['id' => $data['application_fk']]);
            }
            // Division cheif
        ...
        if($data['flag'] == DIVISON_CHIEF) {
            ...
            if($designationType['designation_id'] == REGIONAL_DIRECTOR){
                $notification = ['user_fk'=>$data['user_fk'],'application_fk'=>$data['application_fk'],
                    'sender_id'=>$data['sender_id'],'receiver_id'=>$receiverId,'comments_fk' => $commentId, 
                    'flag_1' => 1,'flag_2' => 1,'flag_3' => 1,'flag_4' => 1,'flag_5' => 0,'date_created'=>$data['date_created']];
            }
        ...
        // For Regional Director.
        if($data['flag'] == REGIONAL_DIRECTOR) {
            // Receiver id.
            $receiverId = $data['receiver_id'];
            $designationType = $this->UserModel->getUserDesignationByEmployeeId($receiverId);
            ...
            if($data['status'] == APPROVED) {
                // For PCO client.
                $app = $this->getApplicationDetails($data['application_fk']); // Get the PCO account id.
                $notification = ['user_fk'=>$data['user_fk'],'application_fk'=>$data['application_fk'],
                    'sender_id'=>$data['sender_id'],'receiver_id'=>$app['account_fk'],'comments_fk' => $commentId, 
                    'flag_1' => 0,'flag_2' => 1,'flag_3' => 1,'flag_4' => 1,'flag_5' => 1,'date_created'=>$data['date_created']];
                $result['application_approved'] = TRUE;
            }
        ...
        $dispositionLog = [
            'application_id' => $data['application_fk'], // Application id.
            'subject' => null, // User Remarks.
            'from' => $this->session->userdata['user']['id'],// Current logged user id.
            'forwarded_to' => $receiverId, // User who is assigned to the application.
        ...
        $this->ApplicationDispositionPageModel->dispositionLog($dispositionLog);
